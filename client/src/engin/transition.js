...
//    var key = string.upper(tostring(easingName))
    var key = tostring(easingName).tolocaleUpperCase();
    ...
    if( ACTION_EASING[key]) {
        var action = ACTION_EASING[key]
        var cls = action[0];
        var count = action[1];
        var defaultT = action[2];
        //var cls, count, default = unpack(ACTION_EASING[key])
        if (count == 2)
            easing = cls.create(action, more ||defaultT)
        else
...
transition.create = function(action, args) {
    if (args == null) args = {};
    if (args.easing) {
        if (isArray(args.easing))
    ...
    var actions = []
    var delay = checknumber(args.delay)
    if (delay > 0)
...
@return mixed 结果
**/
transition.execute=function(target, action, args) {
    var action = this.create(action, args)
...
@return mixed 结果
**/
transition.rotateTo=function(target, args) {
    //var rotation = args.rotate
    var action = cc.RotateTo.create(args.time, args.rotate)
    return this.execute(target, action, args)
}
...
@return mixed 结果
**/
transition.moveTo=function(target, args) {
    var position = target.getPosition();
    var tx = position.x;
    var ty = position.y
    var x = args.x || tx
    var y = args.y || ty
    var action = cc.MoveTo.create(args.time, cc.p(x, y))
    return this.execute(target, action, args)
}
...
@return mixed 结果
**/
transition.moveBy=function(target, args) {
    var x = args.x || 0
    var y = args.y || 0
    var action = cc.MoveBy.create(args.time, cc.p(x, y))
    return this.execute(target, action, args)
}
...
@return mixed 结果
**/
transition.fadeIn=function(target, args) {
    var action = cc.FadeIn.create(args.time)
    return this.execute(target, action, args)
}
...
@return mixed 结果
**/
transition.fadeOut=function(target, args) {
    var action = cc.FadeOut.create(args.time)
    return this.execute(target, action, args)
}
...
@return mixed 结果
**/
transition.fadeTo=function(target, args) {
    var opacity = checkint(args.opacity)
    if (opacity < 0)
        opacity = 0
    else if (opacity > 255)
        opacity = 255

    var action = cc.FadeTo.create(args.time, opacity)
    return this.execute(target, action, args)
}
...
@return mixed 结果
**/
transition.scaleTo=function(target, args) {
    var action
    if (args.scale)
        action = cc.ScaleTo.create(checknumber(args.time), checknumber(args.scale))
    else if (args.scaleX || args.scaleY) {
        var scaleX;
        var scaleY
        if (args.scaleX)
...
**/
transition.sequence=function(actions) {
...
@return table 动作表格对象
**/
transition.playAnimationOnce=function(target, animation, removeWhenFinished, onComplete, delay) {
    var actions = []
...
@return table 动作表格对象
**/
transition.playAnimationForever=function(target, animation, delay) {
...
/**
    ...
    local action = transition.moveTo(sprite, {time = 2.0, x = 100, y = 100})
        ...
        @param mixed target
**/
transition.removeAction=function(action) {
...
@param mixed target
**/
transition.stopTarget=function(target) {
    var actionManager = cc.Director.getActionManager()
    if (target)
...
/**
    ...
    @param mixed target
**/
transition.pauseTarget=function(target) {
    var actionManager = cc.Director.getActionManager()
    if (target)
...
/**
    ...
    @param mixed target
**/
transition.resumeTarget=function(target) {
    var actionManager = cc.Director.getActionManager()
    if (target)
